...
(function () {
    ...
    async function fetchTMDBReviews(itemId) {
        try {
            ...
            const url = window.ApiClient.getUrl(`api/cavea/reviews/${itemId}`);
    ...
    function createReviewsCarousel(reviews) {
        ...
        if (document.querySelector('.cavea-reviews-carousel')) {
        ...
        if (!document.getElementById('cavea-reviews-responsive-style')) {
            const style = document.createElement('style');
            style.id = 'cavea-reviews-responsive-style';
            style.textContent = `
                .cavea-reviews-carousel {
                    padding: 1.5rem 0;
                }
        ...
        wrapper.className = 'cavea-reviews-carousel';
        ...
        prevBtn.className = 'cavea-review-prev';
        ...
        nextBtn.className = 'cavea-review-next';
        ...
        track.className = 'cavea-review-track';
        ...
        reviews.slice(0, 20).forEach((review, idx) => {
            ...
            card.className = 'cavea-review-card';
            card.style.cssText = `
        ...
        dotsContainer.className = 'cavea-review-dots';
        ...
        for (let i = 0; i < reviews.length; i++) {
            ...
            dot.addEventListener('click', () => {
                const firstCard = track.querySelector('.cavea-review-card');
                const gap = 15;
    ...
    function showReviewModal(author, content) {
        ...
        const existing = document.getElementById('cavea-review-modal');
        if (existing) existing.remove();

        const modal = document.createElement('div');
        modal.id = 'cavea-review-modal';
        modal.style.cssText = `
            position: fixed;
            top: 0;
            left: 0;
    ...
    function escapeHtml(text) {
        ...
        popup.className = 'cavea-review-popup';
    ...
    async function initReviewsCarousel() {
        ...
        try {
            const configUrl = window.ApiClient.getUrl('api/cavea/config');
        ...
        const localToggle = localStorage.getItem('cavea_show_reviews');
