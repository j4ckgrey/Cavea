...
(function () {
    ...
    async function fetchMetadata(jellyfinId, card, modal, title, year, forceSeries) {
        ...
        try {
            ...
            if (!finalImdbId && tmdbIdFromResponse) {
                try {
                    const params = new URLSearchParams();
                    ...
                    if (externalData?.imdb_id) {
                        finalImdbId = externalData.imdb_id;
                        modal.dataset.imdbId = finalImdbId;
                        console.log('[DetailsModal] Resolved external IMDb ID:', finalImdbId);
                    } else if (externalData?.id && externalData.id.toString().startsWith('tt')) {
                        finalImdbId = externalData.id.toString();
                        modal.dataset.imdbId = finalImdbId;
                        console.log('[DetailsModal] Resolved external IMDb ID from id field:', finalImdbId);
                    }
                } catch (err) {
            ...
            } else if (tmdbIdFromResponse) {
                const { credits } = await fetchTMDBCreditsAndReviews(actualType === 'series' ? 'tv' : 'movie', tmdbIdFromResponse);
