...
(function () {
	...
	// Global cache for streams: { itemId: { mediaSourceId: { audio: [], subs: [] } } }
	window.CaveaStreamCache = window.CaveaStreamCache || {};
	...
	async function fetchStreams(itemId, mediaSourceId) {
		...
		try {
			...
			const url = window.ApiClient.getUrl('api/cavea/metadata/streams') + '?' + params;
			...
			if (response && response.cache) {
				if (!window.CaveaStreamCache[itemId]) window.CaveaStreamCache[itemId] = {};
				for (const [sid, data] of Object.entries(response.cache)) {
					window.CaveaStreamCache[itemId][sid] = data;
				}
				log('Updated local cache with', Object.keys(response.cache).length, 'sources');
			}

			// Also cache the direct response
			if (response && mediaSourceId) {
				if (!window.CaveaStreamCache[itemId]) window.CaveaStreamCache[itemId] = {};
				window.CaveaStreamCache[itemId][mediaSourceId] = { audio: response.audio, subs: response.subs };
			}
	...
	async function loadTracksForVersion(versionSelect, mediaSourceId) {
		...
		// Check Cache First
		let streams = null;
		if (window.CaveaStreamCache[currentItemId] && window.CaveaStreamCache[currentItemId][mediaSourceId]) {
			streams = window.CaveaStreamCache[currentItemId][mediaSourceId];
			log('Cache HIT for', mediaSourceId);
		} else {
